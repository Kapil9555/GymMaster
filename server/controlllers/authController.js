...
const registerController = async (req, res) => {
    try {
        ...
        if (!/^[A-Za-z ]+$/.test(name.trim())) {
            return res.status(400).json({ success: false, message: "Name must contain only alphabets and spaces" });
        }
        ...
        if (!/^[A-Za-z ]+$/.test(city)) {
            return res.status(400).json({ success: false, message: "City must contain only alphabets and spaces" });
        }
        if (!/^(9|8|7|6)\d{9}$/.test(contact)) {
